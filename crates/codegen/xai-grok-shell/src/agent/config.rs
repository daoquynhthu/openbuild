...
pub fn resolve_model_list(
    ...
    // Layer 2b (removed): provider model discovery via async catalog is the
    // only path. Blocking network fetch was removed in P9-012 — startup
    // merges persisted/in-memory snapshot passed via `prefetched`.
